...
window.CodeMirror = (function() {
  ...
  function makeChangeSingleDocInEditor(cm, change, spans, selAfter) {
    ...
    if (hasHandler(cm, "change")) {
      var changeObj = {from: from, to: to, text: change.text, origin: change.origin,
                      updated: change.updated, removed: change.removed, added: change.added};
  ...
  function findPosH(doc, pos, dir, unit, visually) {
    ...
    else if (unit == "word" || unit == "wordBoundary") {
      var sawWord = false, boundary = unit == "wordBoundary", cur = null;
      for (;;) {
        if (dir < 0 && !moveOnce()) break;
        var wordChar = isWordChar(lineObj.text.charAt(ch)), done = false;
        if (boundary) {
          if (cur == null) cur = wordChar;
          else done = cur != wordChar;
        } else {
          if (wordChar) sawWord = true;
          else done = sawWord;
        }
        if (done) {if (dir < 0) {dir = 1; moveOnce();} break;}
        if (dir > 0 && !moveOnce()) break;
      }
    }
  ...
  var commands = CodeMirror.commands = {
    ...
    goWordBoundaryLeft: function(cm) {cm.moveH(-1, "wordBoundary");},
    goWordBoundaryRight: function(cm) {cm.moveH(1, "wordBoundary");},
  ...
  keyMap.pcDefault = {
    ...
    "Ctrl-Left": "goWordBoundaryLeft", "Ctrl-Right": "goWordBoundaryRight", "Alt-Left": "goLineStart", "Alt-Right": "goLineEnd",
  ...
  keyMap.macDefault = {
    ...
    "Cmd-Up": "goDocStart", "Cmd-End": "goDocEnd", "Cmd-Down": "goDocEnd", "Alt-Left": "goWordBoundaryLeft",
    "Alt-Right": "goWordBoundaryRight", "Cmd-Left": "goLineStart", "Cmd-Right": "goLineEnd", "Alt-Backspace": "delWordBefore",
  ...
  function updateDoc(doc, change, markedSpans, selAfter, estimateHeight) {
    ...
    if (from.ch == 0 && to.ch == 0 && lastText == "") {
      ...
      change.updated = [lastLine];
      change.removed = getLineHandles(doc, from.line, from.line + nlines);
      ...
      change.updated = [firstLine];        
      change.removed = getLineHandles(doc, from.line + 1, from.line + 1 + nlines);
      doc.remove(from.line + 1, nlines);
      change.added = [];
    } else {
      updateLine(firstLine, firstLine.text.slice(0, from.ch) + text[0], spansFor(0), estimateHeight);
      updateLine(lastLine, lastText + lastLine.text.slice(to.ch), lastSpans, estimateHeight);
      change.updated = [firstLine, lastLine];
      for (var i = 1, e = text.length - 1, added = []; i < e; ++i)
        added.push(makeLine(text[i], spansFor(i), estimateHeight));
      change.removed = getLineHandles(doc, from.line + 1, from.line + 1 + nlines - 1);
