...
(function(mod) {
    ...
    transposeChars: function(cm) {
      runInOp(cm, function() {
        ...
        for (var i = 0; i < ranges.length; i++) {
          ...
          if (line.length > 1) {
            if (cur.ch === 0) cur.ch++; 
            if (cur.ch === line.length) cur.ch--; 
          }
          if (cur.ch > 0 && cur.ch < line.length)
  ...
  function highlightLine(cm, line, state, forceToEnd) {
    ...
    if (externalMode) {
      var externalStyles = externalMode(cm, line);
  ...
  function insertLineContent(line, builder, styles) {
    ...
    /*
    BREADKS codemirror span names such as CodeMirror-selectedtext since CM will be prepended to
