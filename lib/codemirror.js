...
window.CodeMirror = (function() {
  ...
  function myCollapsedSpanAt(line, ch) {
  ...
  function updateSelectionRange(cm) {
    ...
    var minRightSelectionEdge = -Infinity;
    var maxLeftSelectionEdge = Infinity;

    minRightSelectionEdge = (clientWidth + defaultCharWidth * 1) - padding;
    maxLeftSelectionEdge = padding;

    function add(left, top, width, bottom, overrideClass) {
      if (top < 0) top = 0;
      var className = overrideClass ? overrideClass : "CodeMirror-selected"; 
      fragment.appendChild(elt("div", null, className, "position: absolute; left: " + left +
    ...
    function drawForLine(line, fromArg, toArg, retTop) {
      ...
      var fromArgResolved = fromArg === null ? 0 : fromArg;
      var toArgResolved = toArg === null ? lineLen : toArg;

      function coords(ch) {
        return charCoords(cm, Pos(line, ch), "div", lineObj);
      }

      iterateBidiSections(getOrder(lineObj), fromArgResolved, toArgResolved, function(from, to, dir) {
        var wrapped = false;
        var p1 = coords(from);
        ...
        for (var i = from + 1; i < to; i++) {
          ...
          if (p1.top === p2.top && p1.bottom === p2.bottom) {
            ...
            p1.right = Math.max(p1.right, p2.right);
          } else {
            if (fromArg === null || wrapped) {
              p1.left = Math.min(maxLeftSelectionEdge, p1.left);
            }
            p1.right = Math.max(minRightSelectionEdge, p1.right);
            add(p1.left, p1.top, p1.right - p1.left, p1.bottom);
            wrapped = true;
            p1 = p2;
          }
        }
        ...
        if (drawSelectedInvisibles) {
          // Draw newlines
          if (toArg === null) {
            var newlinePos = coords(lineLen);
            if (!collapsedSpanAtEnd(lineObj)) {
              add(newlinePos.left, newlinePos.top, newlinePos.right, newlinePos.bottom, 'selected-newline');
            }
          }

          // Draw tabs
          var nextTabIndex = lineText.indexOf('\t', from);
          while (nextTabIndex >= from && nextTabIndex < to) {
            if (!myCollapsedSpanAt(lineObj, nextTabIndex)) {
                var tabPos = coords(nextTabIndex);
                add(tabPos.left, tabPos.top, tabPos.right - tabPos.left, tabPos.bottom, 'selected-tab');
            }
            nextTabIndex = lineText.indexOf('\t', nextTabIndex + 1);
          }

          // Draw Spaces
          var nextSpaceIndex = lineText.indexOf(' ', from);
          while (nextSpaceIndex >= from && nextSpaceIndex < to) {
            if (!myCollapsedSpanAt(lineObj, nextSpaceIndex)) {
              var spacePos = coords(nextSpaceIndex);
              var width = spacePos.right - spacePos.left;
              if (width < defaultCharWidth) {
                width = defaultCharWidth;
              }
              add(spacePos.left, spacePos.top, width, spacePos.bottom, 'selected-space');
            }
            nextSpaceIndex = lineText.indexOf(' ', nextSpaceIndex + 1);            
          }
        }
      });
    }
    ...
    if (sel.from.line == sel.to.line) {
      ...
      var showingFrom = display.showingFrom;
      var showingTo = display.showingTo;
      for (var l = sel.from.line; l <= sel.to.line; l++) {
        if (l >= showingFrom && l <= showingTo) {
          if (l === sel.from.line) {
            drawForLine(l, sel.from.ch, null);        
          } else if (l === sel.to.line) {
            drawForLine(l, null, sel.to.ch);        
          } else {
            if (!lineIsHidden(doc, getLine(doc, l))) {
              drawForLine(l, null, null);        
            }
          }          
        }
      }
