...
(function(mod) {
  ...
  function updateDisplay(cm, viewPort, forced) {
    ...
    var positionLineWidgets = cm.display.positionLineWidgets = [];
    ...
    for (var i = 0; i < positionLineWidgets.length; i++) {
      positionLineWidgets[i].widget.positionWidget(positionLineWidgets[i].widget.node.parentNode);
    }

    cm.display.positionLineWidgets = null;
  ...
  function updateLineClasses(lineView) {
    ...
    var wrapClass = lineView.wrapClass ? lineView.wrapClass + " " + (lineView.line.wrapClass || "") : lineView.line.wrapClass;
    if (wrapClass)
  ...
  function drawSelectionRange(cm, range, output) {
    ...
    if (sFrom.line == sTo.line) {
      ...
      if (fromArg === null && toArg === 0) {
        ...
        rightStart = drawForLine(sTo.line, fromArg, sTo.ch).start;
        if (singleVLine) {
  ...
  function measureCharInner(cm, prepared, ch, bias) {
    ...
    if (left) {
      // jesse hack so that possible to set margins in renderline
      result.left += left;
      result.right += left;      
    }
  ...
  function registerEventHandlers(cm) {
    ...
    function onResize() {
      if (resizeTimer == null) resizeTimer = setTimeout(function() {
        ...
        d.cachedCharWidth = d.cachedSpaceWidth = d.cachedTextHeight = d.cachedPaddingH = knownScrollbarWidth = null;
        cm.setSize();
      }, 100);
    }
    ...
    if (cm.options.dragDrop) {
      ...
      on(d.scroller, "drag", drag_);
      on(d.scroller, "dragend", drag_);
      on(d.scroller, "dragenter", drag_);
      on(d.scroller, "dragover", drag_);
      on(d.scroller, "dragleave", drag_);
  ...
  function onDrop(e) {
    ...
    if (textForDropEvent === null && files && files.length && window.FileReader && window.File) {
      ...
      try {
        var text = textForDropEvent ? textForDropEvent : e.dataTransfer.getData("Text");
        if (text) {
  ...
  function onContextMenu(cm, e) {
    ...
    if (reset && cm.doc.sel.contains(pos) == -1)
      operation(cm, setSelection)(cm.doc, new Selection([findWordAt(cm.doc, pos)], 0), sel_dontScroll);
  ...
  function scrollPosIntoView(cm, pos, end, topMargin, bottomMargin) {
    ...
    for (;;) {
      ...
      var scrollPos = calculateScrollPos(cm, Math.min(coords.left, endCoords.left),
                                         Math.min(coords.top, endCoords.top) - topMargin,
                                         Math.max(coords.left, endCoords.left),
                                         Math.max(coords.bottom, endCoords.bottom) + bottomMargin);
  ...
  function resolveScrollToPos(cm) {
    ...
    if (range) {
      ...
      var sPos = calculateScrollPos(cm, Math.min(from.left, to.left),
                                    Math.min(from.top, to.top) - range.topMargin,
                                    Math.max(from.right, to.right),
                                    Math.max(from.bottom, to.bottom) + range.bottomMargin);
  ...
  CodeMirror.prototype = {
    ...
    scrollIntoView: methodOp(function(range, topMargin, bottomMargin) {
      ...
      range.bottomMargin = bottomMargin === undefined ? topMargin : bottomMargin;
      ...
      if (range.from.line != null) {
        ...
        var sPos = calculateScrollPos(this, Math.min(range.from.left, range.to.left),
                                      Math.min(range.from.top, range.to.top) - range.topMargin,
                                      Math.max(range.from.right, range.to.right),
                                      Math.max(range.from.bottom, range.to.bottom) + range.bottomMargin);
  ...
  function highlightLine(cm, line, state, forceToEnd) {
    ...
    var externalMode = cm.getOption('externalMode');
    if (externalMode) {
      externalMode(cm, line, st);
    } else {
  ...
  function insertLineContent(line, builder, styles) {
    ...
    var attributedString = new builder.cm.options.AttributedString(line.text);
    var start = 0;
    var end = 0;
    var runStyles;
    var runs = [];
    ...
    if (spans) {
      for (var j = 0; j < spans.length; ++j) {
        ...
        if (marker.className) {
          // trim off 'cm-''
          attributedString.addAttributeInRange(marker.className.substr(3), '', from, to - from);
        }
      }      
    }

    styles = [styles[0]];
    attributedString.toCodeMirrorStyleArray(styles, true);
