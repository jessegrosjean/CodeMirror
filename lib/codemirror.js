...
window.CodeMirror = (function() {
  ...
  function updateSelectionRange(cm) {
    ...
    function drawForLine(line, fromArg, toArg, retTop) {
      ...
      var lineLen = lineText.length, rVal = retTop ? Infinity : -Infinity;
      var drawNewline = toArg === -1;
      ...
      if (drawNewline) {
        toArg = lineText.length;
        var newLinePos = coords(toArg);
        add(newLinePos.left, newLinePos.top, defaultCharWidth, newLinePos.bottom);
        add(newLinePos.left, newLinePos.top, defaultCharWidth, newLinePos.bottom, 'selected-newline');
      }
      ...
      iterateBidiSections(getOrder(lineObj), fromArg || 0, toArg == null ? lineLen : toArg, function(from, to, dir) {
        ...
        var nextTabIndex = lineText.indexOf('\t', fromArg);
        while (nextTabIndex >= fromArg && nextTabIndex < toArg) {
        ...
        var nextSpaceIndex = lineText.indexOf(' ', fromArg);
        while (nextSpaceIndex >= fromArg && nextSpaceIndex < toArg) {
    ...
    if (sel.from.line == sel.to.line) {
      ...
      for (var l = sel.from.line; l <= sel.to.line; l++) {
        if (l >= showingFrom && l <= showingTo) {
          if (l === sel.from.line) {
            drawForLine(l, sel.from.ch, -1);        
          } else if (l === sel.to.line) {
            drawForLine(l, 0, sel.to.ch);        
          } else {
            drawForLine(l, 0, -1);        
          }          
        }
