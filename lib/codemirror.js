...
(function(mod) {
  ...
  function measureCharInner(cm, prepared, ch, bias) {
    ...
    if (node.nodeType == 3) { // If it is a text node, use a range to retrieve the coordinates.
      ...
      if (ie && ie_version < 9 && start == 0 && end == mEnd - mStart) {
        ...
        if (rect.width === 0) {
          // Jesse hack. Sometimes, such as when a space is at the end of a
  ...
  function onContextMenu(cm, e) {
    ...
    if (reset && (!cm.doc.sel.somethingSelected() || cm.doc.sel.contains(pos) == -1))
      operation(cm, setSelection)(cm.doc, new Selection([findWordAt(cm, pos)], 0), sel_dontScroll);
